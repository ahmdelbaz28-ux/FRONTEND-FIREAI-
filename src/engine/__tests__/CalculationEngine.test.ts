...
describe('CalculationEngine - Voltage Drop', () => {
  it('should calculate voltage drop for copper cable correctly', () => {
    // Test: 100A over 50m, 2.5mm² Cu cable, PF 0.85, 230V
    // R for 2.5mm² Cu = 7.41 Ω/km, X = 0.135 Ω/km
    // ΔV = I × L × (R × cosφ + X × sinφ) / 1000
    // cosφ = 0.85, sinφ = √(1-0.85²) = 0.527
    // ΔV = 100 × 0.05 × (7.41 × 0.85 + 0.135 × 0.527) = 100 × 0.05 × (6.2985 + 0.0711) = 0.318 V
    // % = 0.318 / 230 × 100 = 0.138%
    
    const result = calculateVoltageDrop(100, 50, 'Cu', 2.5, 0.85, 230);
  ...
  it('should handle different power factors', () => {
    const resultPF1 = calculateVoltageDrop(100, 50, 'Cu', 2.5, 1.0, 230);
    const resultPF08 = calculateVoltageDrop(100, 50, 'Cu', 2.5, 0.8, 230);
    
    // Higher PF = lower voltage drop (less reactive component)
    expect(resultPF1.percentage).toBeLessThan(resultPF08.percentage);
  });
...
describe('CalculationEngine - Short Circuit', () => {
  it('should calculate prospective short circuit current', () => {
    ...
    expect(result.status).toBe('PASS' | 'FAIL'); // Either valid state
  });

  it('should fail when breaker rating is insufficient', () => {
    // Very long cable with small cross-section should fail
    const result = calculateShortCircuit(400, 200, 'Cu', 1.5, 50, 6);
    
    expect(result.minRequiredBreakingCapacity).toBeGreaterThan(result.breakerRating);
    expect(result.status).toBe('FAIL');
  });
