...
export function validateSmokeDetectorPlacement(
  ...
  for (let i = 0; i < detectors.length; i++) {
    ...
    // Check distance from walls (assuming room is centered)
    ...
    if (distFromWall > SMOKE_DETECTOR_MAX_FROM_WALL) {
      results.push({
        isCompliant: false,
        actualDistance: distFromWall,
        maxDistance: SMOKE_DETECTOR_MAX_FROM_WALL,
        violations: [{
          id: 'NFPA-003',
          standard: 'NFPA72',
          rule: '17.4.3(c)',
          section: 'Wall Distance',
          severity: 'WARNING',
          message: `Detector more than ${SMOKE_DETECTOR_MAX_FROM_WALL}m from wall (${distFromWall.toFixed(2)}m)`,
          affectedDevices: [detectors[i].id],
          recommendation: 'Relocate detector closer to wall or add additional detector',
          autoFixable: false
        }]
      });
    }
